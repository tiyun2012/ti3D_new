...
import { Entity, Asset, GraphNode, ComponentType, SelectionType, StaticMeshAsset, PhysicsMaterialAsset } from '../types';
        ...
        <div className="h-full bg-panel flex flex-col font-sans border-l border-black/20">
            ...
            <div className="p-4 space-y-4">
                {asset.type === 'PHYSICS_MATERIAL' && (
                    <>
                        <DraggableNumber label="Static Friction" value={(asset as PhysicsMaterialAsset).data.staticFriction} onChange={v => { assetManager.updatePhysicsMaterial(asset.id, {staticFriction:v}); setRefresh(r=>r+1); }} step={0.05} />
                        <DraggableNumber label="Dynamic Friction" value={(asset as PhysicsMaterialAsset).data.dynamicFriction} onChange={v => { assetManager.updatePhysicsMaterial(asset.id, {dynamicFriction:v}); setRefresh(r=>r+1); }} step={0.05} />
                        <DraggableNumber label="Bounciness" value={(asset as PhysicsMaterialAsset).data.bounciness} onChange={v => { assetManager.updatePhysicsMaterial(asset.id, {bounciness:v}); setRefresh(r=>r+1); }} step={0.05} />
                    </>
                )}
            </div>
        </div>
