...
out vec4 v_weights; // Pass weights to fragment for heatmap

// %VERTEX_LOGIC%

mat4 getBoneMatrix(int jointIndex) {
    // 256x4 texture. Each matrix is 4 pixels. 
    // row 0. 1024 width.
    // Pixel 0,1,2,3 = Matrix 0.
    
    // Simplification: We assume a texture width of 1024.
    // 1 pixel = 1 vec4. 4 pixels = 1 mat4.
    ...
    // Construct column-major matrix from rows if stored as rows, or cols if stored as cols.
    // Usually standard is: row1, row2, row3, row4.
    // GLSL mat4 constructor takes columns.
    // So we must transpose if we stored rows.
...
void main() {
    ...
    if (u_hasSkinning == 1) {
        ...
        // Transform local position by skin matrix
        localPos = skinMatrix * localPos;
        
        // Transform normal (ignoring scale for now)
...
void main() {
    ...
    else if (u_renderMode == 5) { // Heatmap Mode (Skinning)
       // We need to know which bone is selected. 
       // Note: v_joints is not passed, only v_weights. 
       // In a real engine, we'd pass v_joints to FS. 
       // Since this is a single pass, we will assume we visualize the max weight for now or white.
       // IMPROVEMENT: visualize max weight
...
// ... (GRID & PP Shaders omitted for brevity, assumed unchanged) ...
...
export class WebGLRenderer {
    ...
    initBoneTexture(gl: WebGL2RenderingContext) {
        ...
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        // Create 256x4 texture to hold 256 4x4 matrices. 
        // 1024 pixels width * 1 pixel height.
        // Each pixel is RGBA32F.
