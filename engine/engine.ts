...
export class Engine {
    ...
    // Deferred texture uploads until WebGL is initialized
    private pendingTextureUploads: Array<{ layerIndex: number; image: HTMLImageElement }> = [];
    ...
    private initEventListeners() {
        ...
        // Texture uploads are deferred until WebGL is initialized (see initGL)
        eventBus.on('TEXTURE_LOADED', (payload) => {
            const layerIndex = payload.layerIndex as number;
            const image = payload.image as HTMLImageElement;
            if (this.renderer.gl) {
    ...
    notifyMeshGeometryFinalized(entityId: string) {
        ...
        private flushPendingTextures() {
        if (!this.renderer.gl) return;
        if (this.pendingTextureUploads.length === 0) return;
        for (const t of this.pendingTextureUploads) {
            this.meshSystem.uploadTexture(t.layerIndex, t.image);
        }
...
initGL(canvas: HTMLCanvasElement) {
        ...
        // Upload any textures that were loaded before WebGL was ready
